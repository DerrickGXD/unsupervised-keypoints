...
class TigDogDataset_Final(Dataset):
    ...
    def __getitem__(self, idx_loader):
        ...
        #     for t_ in range(mask_part.shape[0]):
        #         kp = landmarks[t_].astype(np.int)
        #         mask = segmentations[t_]
        #         mask_kp = np.ones_like(mask).astype(np.int)
        #         mask_kp[kp[:, 1], kp[:, 0]] = 0
        #
        #         dist_kp, inds = bwdist(mask_kp, return_indices=True)
        #         for i, k_ in enumerate(kp):
        #             if k_[-1]:
        #                 locs = (inds[0] == k_[1]) & (inds[1] == k_[0])
        #                 mask_part[t_] += locs.astype(np.int) * mask.astype(np.int) * (i + 1)
